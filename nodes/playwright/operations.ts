...
async function prepareBinaryFromResponse(executeFunctions: IExecuteFunctions, response: Response) {
	const headers = response.headers();
	const contentDisposition = headers['content-disposition'] || '';
...
async function fetchFileFromUrl(
	...
	const fileName =
		filenameFromDisposition(contentDisposition) || fallbackFileName || filenameFromUrl(url);
...
async function handleDownloadFromElement(
	...
	const responsePromise = safely(
		page.waitForResponse((response) => looksLikeDownloadResponse(response), {
			timeout: waitTimeout,
		}),
	);
	...
	if (absoluteHref && !absoluteHref.startsWith('javascript:')) {
		const fetchedInPage = await safely(fetchFileThroughPage(executeFunctions, page, absoluteHref));
		...
		const fetchedByRequest = await safely(fetchFileFromUrl(executeFunctions, page, absoluteHref));
	...
	if (popupPage) {
		const popupResponse = await safely(
			popupPage.waitForResponse((response) => looksLikeDownloadResponse(response), {
				timeout: waitTimeout,
			}),
		);
		...
		if (popupUrl && !isViewerLikeUrl(popupUrl)) {
			...
			if (
				...
				const fileName = filenameFromDisposition(contentDisposition) || filenameFromUrl(popupUrl);
	...
	switch (operation) {
		...
		case 'takeScreenshot': {
			...
			const dataPropertyName =
				(executeFunctions.getNodeParameter('dataPropertyName', itemIndex) as string) ||
				'screenshot';
			const screenshot = await page.screenshot(
				screenshotOptions as Parameters<Page['screenshot']>[0],
			);
		...
		case 'getText': {
			const { selectorType, selector, locator } = getActionLocator(
				executeFunctions,
				itemIndex,
				page,
			);
