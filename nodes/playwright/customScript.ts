	...
	const context = {
		...
		helpers: {
			...
			httpRequestWithAuthentication:
				executeFunctions.helpers.httpRequestWithAuthentication?.bind(executeFunctions),
			requestWithAuthenticationPaginated:
				executeFunctions.helpers.requestWithAuthenticationPaginated?.bind(executeFunctions),
		},
	...
	vm.on(
		...
		executeFunctions.getMode() === 'manual'
			...
			: CODE_ENABLE_STDOUT === 'true'
				? (...args: unknown[]) =>
						console.log(
							`[Workflow "${executeFunctions.getWorkflow().id}"][Node "${executeFunctions.getNode().name}"]`,
							...args,
						)
				: () => {},
	);
